...
describe("TakepileToken leverage with fees", function () {
  let underlyingToken,
    takeToken,
    pileToken,
    takepileFactory,
  ...
  describe("Entering / Exiting positions", function () {
    it("Should fully exit leveraged position with no price change", async function () {
      ...
      expect(await pileToken.balanceOf(aliceAddr)).to.be.eq(dollar(895));

      // Position size 200 * .999 = 199.8
      // Fees = 200 - 199.8 = 0.2
      // Collateral = 100 - fees = 99.8
      expect((await pileToken.positions(aliceAddr, 'ETHUST')).amount).to.be.eq(dollar(199.8));
      expect((await pileToken.positions(aliceAddr, 'ETHUST')).collateral).to.be.eq(dollar(104.8));
      ...
      expect(await pileToken.balanceOf(aliceAddr)).to.be.eq(dollar(999.60020));
    ...
    it("Should partially exit leveraged position with no price change", async function () {
      ...
      expect(await pileToken.balanceOf(aliceAddr)).to.be.eq(dollar(899));

      // Position size 200 * .999 = 199.8
      // Fees = 200 - 199.8 = 0.2
      // Collateral = 100 - fees = 99.8
      expect((await pileToken.positions(aliceAddr, 'ETHUST')).amount).to.be.eq(dollar(199.8));
      expect((await pileToken.positions(aliceAddr, 'ETHUST')).collateral).to.be.eq(dollar(100.8));
      ...
      expect((await pileToken.positions(aliceAddr, 'ETHUST')).amount).to.be.eq(dollar(99));
      expect((await pileToken.positions(aliceAddr, 'ETHUST')).collateral).to.be.eq(dollar(51));
      expect(await pileToken.balanceOf(aliceAddr)).to.be.eq(dollar(948.6992)); // 899 + 49.6992
    ...
    it("Should allow adding collateral with no position size change", async () => {
      ...
      expect((await pileToken.positions(aliceAddr, 'ETHUST')).amount).to.be.eq(dollar(199.8));
      expect((await pileToken.positions(aliceAddr, 'ETHUST')).collateral).to.be.eq(dollar(200.8));
